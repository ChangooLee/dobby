...
function addTranscript(role: "user" | "dobi", text: string) {
  ...
  // 최대 N개 유지
  while (transcriptLog.children.length > MAX_ENTRIES) {
...
const voiceInput = createVoiceInput(
  (text: string) => {
    // Cancel any current DOBBY response before sending new input
    audioPlayer.stop();
...
btnFixSelf.addEventListener("click", (e) => {
  e.stopPropagation();
  menuDropdown.style.display = "none";
  // Activate work mode on the WebSocket session (DOBBY becomes Claude Code's voice)
