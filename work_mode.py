"""
DOBBY Work Mode — persistent claude -p sessions tied to projects.

DOBBY can connect to any project directory and maintain a conversation
...
DOBBY reads the responses via subprocess, summarizes, and reports back.
"""
...
log = logging.getLogger("dobby.work_mode")
...
class WorkSession:
    ...
    Each project gets its own session. DOBBY can switch between projects
    ...
    async def send(self, user_text: str) -> str:
        ...
        # claude는 VM 기반 바이너리라 zsh를 통해야 실행됨
        continue_flag = "--continue" if self._message_count > 0 else ""
        safe_text = user_text.replace("'", "'\\''")
        shell_cmd = (
            f"claude -p --output-format text {continue_flag} '{safe_text}'"
        )
        ...
        try:
            ...
            return "예상보다 오래 걸리고 있습니다, 주인님. 작업이 시간 초과되었습니다."
        except Exception as e:
            log.error(f"Work mode error: {e}")
            self._status = "error"
            return f"오류가 발생했습니다, 주인님: {str(e)[:100]}"
