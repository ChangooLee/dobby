"""
DOBBY Conversation Monitor

Watches the DOBBY server logs in real-time, analyzes conversation quality,
and reports issues that need fixing. Run alongside the DOBBY server.
...
class ConversationMonitor:
    ...
    def analyze_latest(self):
        ...
        # ── Check DOBBY responses ──
        if latest["role"] == "dobby":
            text = latest["text"]

            # Too long for voice?
            sentences = text.split(". ")
            if len(sentences) > 4:
                self.flag(f"DOBBY response too long for voice ({len(sentences)} sentences): {text[:80]}...")

            # Generic AI patterns that DOBBY shouldn't use
            bad_patterns = [
                ("How can I help", "DOBBY doesn't ask 'how can I help' — he just acts"),
                ("Is there anything else", "DOBBY doesn't ask 'is there anything else'"),
                ("I'd be happy to", "Too corporate — DOBBY says 'Will do, sir' or just does it"),
                ("Absolutely!", "DOBBY doesn't use filler enthusiasm"),
                ("Great question", "DOBBY never says 'great question'"),
                ("I don't have access", "DOBBY should say 'I'm afraid I don't have that information, sir'"),
                ("As an AI", "DOBBY never breaks character"),
                ("I cannot", "DOBBY says 'I'm afraid that's beyond my current capabilities, sir'"),
            ]
            ...
            dobby_msgs = [m for m in self.messages if m["role"] == "dobby"]
            if len(dobby_msgs) >= 5:
                recent = dobby_msgs[-5:]
                sir_count = sum(1 for m in recent if "sir" in m["text"].lower())
                if sir_count < 1:
                    self.flag("DOBBY hasn't said 'sir' in the last 5 responses — should use it more")
            ...
            if prev and prev["role"] == "user":
                ...
                if any(w in user_text for w in ["earlier", "before", "you said", "we talked about", "remember"]):
                    if "I don't recall" in text or "I'm not sure what" in text:
                        self.flag("DOBBY failed to recall earlier conversation — memory issue")

            # Response references Samantha?
            if "samantha" in text.lower():
                self.flag("DOBBY referenced 'Samantha' — should never mention her, he IS the assistant")
        ...
        if latest["role"] == "user":
            ...
            for pattern in complaint_patterns:
                if pattern in text:
                    self.flag(f"USER COMPLAINT detected: '{pattern}' — review DOBBY's previous response")
...
def main():
    ...
    print("🔍 DOBBY Conversation Monitor")
    ...
    try:
        for line in sys.stdin:
            ...
            # Parse DOBBY responses
            dobby_match = re.search(r"DOBBY: (.+)$", line)
            if dobby_match:
                text = dobby_match.group(1)
                print(f"🤖 {text[:80]}{'...' if len(text) > 80 else ''}")
                monitor.add_message("dobby", text)
