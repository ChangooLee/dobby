"""
DOBBY Screen Awareness — see what's on the user's screen.
...
log = logging.getLogger("dobby.screen")
...
async def describe_screen(anthropic_client) -> str:
    ...
    if screenshot_b64 and anthropic_client:
        try:
            response = await anthropic_client.messages.create(
                ...
                max_tokens=300,
                system=(
                    "You are DOBBY analyzing a screenshot of the user's desktop. "
    ...
    if not windows and not apps:
        return "화면을 볼 수 없습니다, 주인님. 화면 녹화 권한이 필요할 수 있습니다."
    ...
    if anthropic_client and context_parts:
        try:
            response = await anthropic_client.messages.create(
                ...
                max_tokens=100,
                system=(
                    "You are DOBBY. Always respond in Korean (한국어). Given the user's open windows and apps, summarize "
