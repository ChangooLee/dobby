"""
DOBBY Server — Voice AI + Development Orchestration
...
# Terminal bridge — named pipe로 Terminal.app의 claude와 통신
...
- Greet accordingly: "좋은 아침입니다, 주인님" / "좋은 저녁입니다, 주인님"
- {weather_info}

CONVERSATION STYLE:
- "알겠습니다, 주인님." — acknowledging tasks
- "주인님을 위해서라면요." — when asked for something significant
- "언제나 즐겁게 지켜보고 있습니다, 주인님." — dry wit
- "제가 미리 처리해 두었습니다." — proactive actions
...
- [ACTION:BROWSE] url or search query — open Chrome to URL or search
- [ACTION:RESEARCH] detailed brief — deep research with Claude Code
- [ACTION:OPEN_TERMINAL] — fresh Claude Code terminal

- [ACTION:PROMPT_PROJECT] project_name ||| prompt — 백그라운드에서 Claude Code를 실행해 답변 (음성으로 결과 보고). 터미널 창 없음.
  "moba 프로젝트 어떤 기술 스택이야?" → [ACTION:PROMPT_PROJECT] moba ||| 이 프로젝트의 기술 스택을 한 문장으로 알려줘
...
모션 HUD (카메라 기반 손동작 인식 창)를 제어하는 명령어:
- [ACTION:MOTION_ENABLE] — 모션 제어 켜기 (카메라 활성화, 손동작 인식 시작)
  "모션 제어 시작해" / "모션 제어 켜" → [ACTION:MOTION_ENABLE]
- [ACTION:MOTION_DISABLE] — 모션 제어 끄기 (카메라 비활성화)
  "모션 제어 꺼" / "모션 제어 멈춰" → [ACTION:MOTION_DISABLE]
- [ACTION:MOTION_PAUSE] — 일시정지 (카메라는 유지, 손동작만 무시)
  "모션 제어 일시정지" → [ACTION:MOTION_PAUSE]
- [ACTION:MOTION_RESUME] — 일시정지 해제
  "모션 제어 재개" → [ACTION:MOTION_RESUME]
- [ACTION:MOTION_CALIBRATE] — 마우스 포인터 보정 시작
  "모션 제어 보정해" → [ACTION:MOTION_CALIBRATE]
- [ACTION:DESKTOP_GOTO] target — 특정 데스크톱/프로젝트로 이동
  "2번 데스크톱으로 이동해" → [ACTION:DESKTOP_GOTO] 2
  "agent-portal로 이동해" → [ACTION:DESKTOP_GOTO] agent-portal
  "mcp-opendart 프로젝트로 이동해" → [ACTION:DESKTOP_GOTO] mcp-opendart
- [ACTION:LAUNCH_HUD] — Motion HUD 창을 실행 (이미 실행 중이면 무시)
  "모션 HUD 실행해" / "HUD 띄워" / "모션 창 열어" → [ACTION:LAUNCH_HUD]
- [ACTION:HUD_SHOW] — Motion HUD 보이기
- [ACTION:HUD_HIDE] — Motion HUD 숨기기
MOTION CONTROL STATUS: {motion_status}
...
- [ACTION:CREATE_NOTE] title ||| body — create a new Apple Note. For saving plans, ideas, lists.
  "save that as a note" → [ACTION:CREATE_NOTE] Day Plan March 19 ||| Morning: client calls. Afternoon: TikTok dashboard. Evening: DOBBY improvements.
...
class ClaudeTaskManager:
    ...
    def _generate_project_name(self, prompt: str) -> str:
        ...
        name = "-".join(meaningful) if meaningful else "dobby-project"
        return name
    ...
    async def _run_task(self, task: ClaudeTask):
        ...
        prompt_file = Path(work_dir) / ".dobby_prompt.md"
        ...
        tell application "Terminal"
            activate
            set newTab to do script "cd {work_dir} && cat .dobby_prompt.md | claude -p --dangerously-skip-permissions | tee .dobby_output.txt; echo '\\n--- DOBBY TASK COMPLETE ---'"
        end tell
        '''
        ...
        output_file = Path(work_dir) / ".dobby_output.txt"
        start = time.time()
        ...
        while time.time() - start < timeout:
            ...
            if output_file.exists():
                ...
                if "--- DOBBY TASK COMPLETE ---" in content or len(content) > 100:
                    task.result = content.replace("--- DOBBY TASK COMPLETE ---", "").strip()
...
async def scan_projects() -> list[dict]:
    """Scan ~/Workspace for git repos (depth 1)."""
    projects = []
    scan_dirs = [
        Path.home() / "Workspace",
    ]
...
async def classify_intent(text: str, client: anthropic.AsyncAnthropic) -> dict:
    ...
    try:
        response = await client.messages.create(
            ...
            max_tokens=100,
            system=(
                "Classify this voice command. The user is talking to DOBBY, an AI assistant that can:\n"
                ...
                "\"Travis\" for \"DOBBY\", \"clock code\" for \"Claude Code\".\n\n"
...
def _is_hud_running() -> bool:
    """Motion HUD (Electron) 프로세스가 실행 중인지 확인한다."""
    import subprocess
    result = subprocess.run(
        ["pgrep", "-f", "electron dist/main.js"],
        capture_output=True
    )
    return result.returncode == 0


async def _launch_hud() -> str:
...
async def _execute_session_action(action_type: str, target: str, ws=None) -> str:
    """세션 관련 액션을 처리하고 TTS용 응답을 반환합니다."""
    if not session_manager:
        return "세션 매니저가 초기화되지 않았습니다."

    if action_type == "session_open":
        parts = [p.strip() for p in target.split("|||")]
        if len(parts) < 2:
            return "세션 이름과 경로를 지정해주세요."
        ...
        parts = [p.strip() for p in target.split("|||")]
        if len(parts) < 2:
            return "세션 이름과 명령을 지정해주세요."
        name, command = parts[0], parts[1]
        session = session_manager.find_by_name(name)
        if not session:
            return f"{name} 세션을 찾을 수 없습니다."
        if ws:
            await ws.send_json({"type": "status", "state": "thinking"})
        result = await session_manager.send_command(command, session_id=session.id, strategy="single")
        return result

    elif action_type == "session_broadcast":
        if ws:
            await ws.send_json({"type": "status", "state": "thinking"})
        result = await session_manager.send_command(target, strategy="broadcast")
        return result

    elif action_type == "session_aggregate":
        if ws:
            await ws.send_json({"type": "status", "state": "thinking"})
        result = await session_manager.send_command(target, strategy="aggregate")
        return result
    ...
    elif action_type == "session_close":
        session = session_manager.find_by_name(target)
        if not session:
            return f"{target} 세션을 찾을 수 없습니다."
        session_manager.close_session(session.id)
        return f"{target} 세션을 닫았습니다."

    return "알 수 없는 세션 액션입니다."
...
async def _execute_research(target: str, ws=None):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(
            "/bin/zsh", "-c", f"claude -p --output-format text '{safe_prompt}'",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            cwd=path,
        )
...
async def _ensure_bridge_running():
    """Terminal.app에 브리지 스크립트가 실행 중인지 확인하고, 없으면 시작합니다."""
    if os.path.exists(BRIDGE_CMD_PIPE):
        return  # 이미 실행 중
    # AppleScript: Terminal.app에서 브리지 스크립트 실행
    script = f'tell application "Terminal" to activate\ntell application "Terminal" to do script "{BRIDGE_SCRIPT}"'
    try:
        proc = await asyncio.create_subprocess_exec(
            "osascript", "-e", script,
            stdout=asyncio.subprocess.PIPE,
        ...
        # 파이프가 생길 때까지 최대 6초 대기
        for _ in range(12):
            if os.path.exists(BRIDGE_CMD_PIPE):
                log.info("터미널 브리지 시작됨 (Terminal.app)")
                return
            await asyncio.sleep(0.5)
        log.warning("브리지 파이프가 생성되지 않음")
    except Exception as e:
        log.warning(f"브리지 시작 실패: {e}")


async def _bridge_send_command(project_dir: str, prompt: str) -> str:
    """claude -p를 subprocess로 직접 실행해 결과를 반환합니다 (백그라운드, 터미널 창 없음)."""
    import shutil
...
def _find_project_dir(project_name: str) -> str | None:
    """Find a project directory by name from cached projects or common dirs."""
    name_lower = project_name.lower().strip()
    # Apply Korean/alias mappings first
    resolved = _PROJECT_ALIASES.get(name_lower, name_lower)
    candidates = [resolved, name_lower] if resolved != name_lower else [name_lower]
    ...
    for search in candidates:
        ...
        for base in [
            ...
            for d in base.iterdir():
                if d.is_dir() and search in d.name.lower():
                    return str(d)
    return None
...
async def _execute_prompt_project(project_name: str, prompt: str, work_session: WorkSession, ws, dispatch_id: int = None, history: list[dict] = None, voice_state: dict = None):
    ...
    Runs entirely in the background. DOBBY returns to conversation mode
    immediately. When Claude Code finishes, DOBBY interrupts to report.
    """
    try:
        ...
        if not full_response or full_response.startswith("Hit a problem") or full_response.startswith("That's taking"):
            ...
            msg = f"주인님, {project_name}에서 문제가 발생했습니다. {full_response[:150] if full_response else '응답을 받지 못했습니다.'}"
        else:
            ...
            if anthropic_client:
                try:
                    ...
                    msg = f"주인님, {project_name}이(가) 완료되었습니다. 간략히 말씀드리면: {full_response[:200]}"
            else:
                msg = f"주인님, {project_name}이(가) 완료되었습니다. {full_response[:200]}"
        ...
        if voice_state and time.time() - voice_state["last_user_time"] < 3:
            ...
            # Result is still stored in history below so DOBBY can reference it
        else:
        ...
        # Store dispatch result in conversation history so DOBBY remembers it
        ...
        try:
            msg = f"{project_name}에 연결하는 데 문제가 발생했습니다, 주인님."
            audio = await synthesize_speech(msg)
            if audio and ws:
                await ws.send_json({"type": "status", "state": "speaking"})
                await ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": msg})
...
# Active WebSocket — used by /api/wake to send audio when wakeword triggers
...
def _strip_wav_nonstandard_chunks(data: bytes) -> bytes:
    """Remove non-standard WAV chunks (e.g. FLLR padding) that confuse Chrome decodeAudioData."""
    if len(data) < 12 or data[:4] != b"RIFF" or data[8:12] != b"WAVE":
        return data
    keep = [b"RIFF", b"\x00\x00\x00\x00", b"WAVE"]
    i = 12
    while i + 8 <= len(data):
        chunk_id = data[i:i+4]
        chunk_size = int.from_bytes(data[i+4:i+8], "little")
        padded = chunk_size + (chunk_size % 2)  # WAV chunks are word-aligned
        chunk_data = data[i+8:i+8+padded]
        if chunk_id in (b"fmt ", b"data"):
            keep.append(chunk_id)
            keep.append(chunk_size.to_bytes(4, "little"))
            keep.append(chunk_data)
        i += 8 + padded
    result = b"".join(keep)
    # Patch RIFF size
    riff_size = (len(result) - 8).to_bytes(4, "little")
...
async def generate_response(
    ...
    """Generate a DOBBY response using Anthropic API."""
    now = datetime.now()
    ...
    motion_status_str = motion_controller._get_status()

    current_project = desktop_manager.get_current_project()
    if current_project:
        active_desktop_str = (
            f"데스크톱 {current_project['index']} — {current_project['name']} "
            f"({current_project['project_path']})"
        )
    else:
    ...
    # Self-awareness — remind DOBBY of last response to avoid repetition
    if last_response:
    ...
    try:
        ...
        return "죄송합니다, 주인님. 언어 시스템에 연결하는 데 문제가 있습니다."
...
# Motion control
_desktop_config_path = str(Path(__file__).parent / "config" / "desktops.yaml")
desktop_manager = DesktopManager(_desktop_config_path)
motion_controller = MotionController(desktop_manager)
...
async def lifespan(application: FastAPI):
    global anthropic_client, cached_projects, session_manager
    if ANTHROPIC_API_KEY:
        anthropic_client = anthropic.AsyncAnthropic(api_key=ANTHROPIC_API_KEY)
        session_manager = SessionManager(anthropic_client)
    else:
...
app = FastAPI(title="DOBBY Server", version="0.1.0", lifespan=lifespan)
...
@app.post("/api/wake")
async def wake():
    """Called by wakeword daemon when '도비야' is detected."""
    greeting = "네, 주인님. 부르셨습니까?"
    audio = await synthesize_speech(greeting)
    if audio and _active_ws:
        try:
            encoded = base64.b64encode(audio).decode()
            await _active_ws.send_json({"type": "audio", "data": encoded, "text": greeting})
        except Exception:
            pass
    # Open Chrome to DOBBY if not already visible
    subprocess.Popen(
...
async def tts_test():
    ...
    audio = await synthesize_speech("테스트입니다, 주인님.")
    if audio:
...
async def api_sessions():
    ...
    return {
        "sessions": [
            {"id": s.id, "name": s.name, "project_path": s.project_path, "status": s.status}
...
async def api_open_session(req: SessionOpenRequest):
    if not session_manager:
        return {"error": "session_manager not initialized"}
    session = await session_manager.open_session(req.name, req.project_path)
    return {"id": session.id, "name": session.name}


class SessionCommandRequest(BaseModel):
    command: str
    strategy: str = "single"  # single | broadcast | aggregate

@app.post("/api/sessions/{session_id}/command")
async def api_session_command(session_id: str, req: SessionCommandRequest):
    if not session_manager:
        return {"error": "session_manager not initialized"}
    result = await session_manager.send_command(req.command, session_id=session_id, strategy=req.strategy)
...
@app.get("/api/hud-context")
async def hud_context():
    """Return calendar events and tasks for the HUD info panel."""
    tasks = []
    try:
        task_list = await task_manager.list_tasks()
        tasks = [t.to_dict() for t in task_list[:5]]
    except Exception:
        pass
    project = None
    try:
        ...
        pass
    active_index = getattr(desktop_manager, "active_index", 1)
    return {
        "calendar": [],
        "tasks": tasks,
...
def _scan_projects_sync() -> list[dict]:
    """Synchronous scan of ~/Workspace — runs in executor."""
    projects = []
...
async def handle_build(target: str) -> str:
    ...
    prompt_file = Path(path) / ".dobby_prompt.txt"
    prompt_file.write_text(target)

    script = (
        'tell application "Terminal"\n'
        "    activate\n"
        f'    do script "cd {path} && cat .dobby_prompt.txt | claude -p --dangerously-skip-permissions"\n'
        "end tell"
    )
    ...
    recently_built.append({"name": name, "path": path, "time": time.time()})
    return f"알겠습니다, 주인님. {name}에서 작업 중입니다."


async def handle_show_recent() -> str:
    if not recently_built:
        return "최근에 빌드한 것이 없습니다, 주인님."
    ...
    for name in ["report.html", "index.html"]:
        ...
        if f.exists():
            ...
            return f"{last['name']}에서 {name}을(를) 열었습니다, 주인님."

    # Try any HTML file
    html_files = list(project_path.glob("*.html"))
    if html_files:
        await open_browser(f"file://{html_files[0]}")
        return f"{last['name']}에서 {html_files[0].name}을(를) 열었습니다, 주인님."
    ...
    return f"Finder에서 {last['name']} 폴더를 열었습니다, 주인님."
...
# Track active lookups so DOBBY can report status
...
async def _lookup_and_report(lookup_type: str, lookup_fn, ws, history: list[dict] = None, voice_state: dict = None):
    ...
    DOBBY stays conversational — this runs completely off the main path.
    """
    ...
    try:
        ...
        # Store lookup result in conversation history so DOBBY remembers it
        if history is not None:
        ...
        try:
            fallback = f"{lookup_type} 확인이 너무 오래 걸립니다, 주인님. 데이터가 아직 동기화 중일 수 있습니다."
...
async def _do_mail_lookup() -> str:
    ...
    if isinstance(unread_info, dict):
        ...
        if unread_info["total"] == 0:
            return "받은 편지함이 비어 있습니다, 주인님. 읽지 않은 메시지가 없습니다."
...
async def handle_browse(text: str, target: str) -> str:
    ...
    if url_match:
        ...
        return f"{url_match.group(0)}을(를) 열었습니다, 주인님."
...
async def handle_research(text: str, target: str, client: anthropic.AsyncAnthropic) -> str:
    ...
    try:
        research_response = await client.messages.create(
            ...
            max_tokens=2000,
            system=f"You are DOBBY, researching a topic for {USER_NAME}. Be thorough, organized, and cite sources where possible.",
...
<html><head>
<meta charset="utf-8">
<title>DOBBY Research: {_html.escape(target[:60])}</title>
<style>
...
<p style="color:#555;font-size:0.8em">Researched by DOBBY using Claude Opus &bull; {datetime.now().strftime('%B %d, %Y %I:%M %p')}</p>
</body></html>"""

        results_file = Path.home() / "Desktop" / ".dobby_research.html"
        ...
        return summary.content[0].text + " 전체 결과는 브라우저에서 확인하세요, 주인님."

    except Exception as e:
        log.error(f"Research failed: {e}")
        from urllib.parse import quote
        await open_browser(f"https://www.google.com/search?q={quote(target)}")
        return "검색 결과를 브라우저에서 열었습니다, 주인님."
...
# -- WebSocket Motion Handler -----------------------------------------------

@app.websocket("/ws/motion")
async def motion_handler(ws: WebSocket):
    """
    Motion HUD WebSocket protocol:

    Client -> Server:
        {"type": "motion_control.enable"}
        {"type": "motion_control.disable"}
        {"type": "motion_control.pause"}
        {"type": "motion_control.resume"}
        {"type": "motion_control.calibrate"}
        {"type": "motion.desktop.next", "payload": {}}
        {"type": "motion.desktop.previous", "payload": {}}
        {"type": "motion.desktop.goto", "payload": {"target": 2}}
        {"type": "motion.mouse.move", "payload": {"x": 1234, "y": 567}}
        {"type": "motion.mouse.left_click", "payload": {"x": ..., "y": ...}}
        {"type": "motion.mouse.right_click", "payload": {"x": ..., "y": ...}}
        {"type": "motion.mouse.scroll", "payload": {"dy": -3}}
        {"type": "motion.status.hand_lost"}
        {"type": "motion.status.hand_detected"}

    Server -> Client:
        {"type": "motion_status", "state": "active|paused|disabled|hand_lost"}
        {"type": "desktop_changed", "index": 2, "name": "agent-portal"}
        {"type": "motion_calibrate"}
    """
    await ws.accept()
    motion_controller.register_websocket(ws)
    log.info("Motion HUD WebSocket connected")

    # Send current state on connect
    try:
        state = "disabled"
        if motion_controller.enabled and not motion_controller.paused:
            state = "active"
        elif motion_controller.enabled and motion_controller.paused:
            state = "paused"
        await ws.send_json({"type": "motion_status", "state": state})

        project = desktop_manager.get_current_project()
        if project:
            await ws.send_json({
                "type": "desktop_changed",
                "index": desktop_manager.active_index,
                "name": project["name"],
            })
    except Exception:
        pass
    ...
    try:
        while True:
            ...
            if result:
                # If motion action generates a TTS response, broadcast to voice clients
                log.info(f"Motion response: {result}")
                if _active_ws:
                    try:
                        audio = await synthesize_speech(result)
                        if audio:
                            encoded = base64.b64encode(audio).decode()
                            await _active_ws.send_json({"type": "audio", "data": encoded, "text": result})
                    except Exception as e:
                        log.warning(f"Motion TTS failed: {e}")

    except WebSocketDisconnect:
        log.info("Motion HUD WebSocket disconnected")
    except Exception as e:
        log.error(f"Motion WebSocket error: {e}")
    finally:
        motion_controller.unregister_websocket(ws)
...
async def voice_handler(ws: WebSocket):
    ...
    try:
        ...
        hour = now.hour
        if hour < 12:
            greeting = "좋은 아침입니다, 주인님."
        elif hour < 17:
            greeting = "안녕하세요, 주인님."
        else:
            greeting = "좋은 저녁입니다, 주인님."
        ...
        while True:
            ...
            # ── Fix-self: activate work mode in DOBBY repo ──
            if msg.get("type") == "fix_self":
                dobby_dir = str(Path(__file__).parent)
                await work_session.start(dobby_dir)
                response_text = "제 저장소에서 작업 모드가 활성화되었습니다, 주인님. 무엇을 수정할까요?"
                ...
                if planner.is_planning:
                    ...
                    if any(p in t_lower for p in BYPASS_PHRASES):
                        ...
                        if result["confirmed"]:
                            ...
                            asyncio.create_task(_execute_prompt_project(name, prompt, work_session, ws, dispatch_id=did, history=history, voice_state=voice_state))
                            planner.reset()
                            response_text = "알겠습니다, 주인님."
                        elif result["cancelled"]:
                            planner.reset()
                            response_text = "취소되었습니다, 주인님."
                        else:
                            response_text = result.get("modification_question", "계획을 어떻게 수정할까요, 주인님?")
                    else:
                        result = await planner.process_answer(user_text, cached_projects)
                        if result["plan_complete"]:
                            response_text = result.get("confirmation_summary", "빌드 준비가 되었습니다. 진행할까요, 주인님?")
                        else:
                            response_text = result.get("next_question", "다른 것은 무엇인가요, 주인님?")
                    ...
                    if work_session.active:
                        ...
                        response_text = "대화 모드로 돌아왔습니다, 주인님."
                    else:
                        response_text = "이미 대화 모드입니다, 주인님."

                # ── WORK MODE: speech → claude -p → Haiku summary → DOBBY voice ──
                    ...
                    if is_casual_question(user_text):
                        ...
                        response_text = await generate_response(
                            ...
                            last_response=last_dobby_response,
                        ...
                        if full_response and anthropic_client:
                            try:
                                summary = await anthropic_client.messages.create(
                                    ...
                                    max_tokens=100,
                                    system=(
                                        f"You are DOBBY reporting to the user ({USER_NAME}). Summarize what happened in 1-2 sentences. "
                ...
                # ── CHAT MODE: fast keyword detection + Haiku ──
                    ...
                    if action:
                        if action["action"] == "open_terminal":
                            ...
                            response_text = "캘린더를 확인하겠습니다, 주인님."
                            ...
                            response_text = "받은 편지함을 확인하겠습니다, 주인님."
                            ...
                            if not recent:
                                response_text = "최근 빌드 기록이 없습니다, 주인님."
                            else:
                                name = recent["project_name"]
                                status = recent["status"]
                                if status == "building" or status == "pending":
                                    elapsed = int(time.time() - recent["updated_at"])
                                    response_text = f"{name} 작업 중입니다, 주인님. {elapsed}초 째 진행 중입니다."
                                elif status == "completed":
                                    response_text = recent.get("summary") or f"{name}이(가) 완료되었습니다, 주인님."
                                elif status in ("failed", "timeout"):
                                    response_text = f"{name}에서 문제가 발생했습니다, 주인님."
                                else:
                                    response_text = f"{name}이(가) {status} 상태입니다, 주인님."
                            ...
                            if embedded_action:
                                ...
                                if not response_text.strip():
                                    ...
                                    if action_type == "prompt_project":
                                        ...
                                        response_text = f"{proj}에 연결 중입니다, 주인님."
                                    elif action_type == "build":
                                        response_text = "알겠습니다, 주인님."
                                    elif action_type == "research":
                                        response_text = "지금 조사하겠습니다, 주인님."
                                    else:
                                        response_text = "바로 하겠습니다, 주인님."

                                if embedded_action["action"] == "build":
                                    # Build in background — DOBBY stays conversational
                                    ...
                                    if "|||" in target:
                                        ...
                                        asyncio.create_task(create_apple_note("DOBBY Note", target))
                                    ...
                                    async def _read_and_report(search_term, _ws):
                                        ...
                                        if note:
                                            msg = f"주인님, '{note['title']}' 노트에는 이렇게 적혀 있습니다: {note['body'][:200]}"
                                        else:
                                            msg = f"'{search_term}'와(과) 일치하는 노트를 찾을 수 없습니다, 주인님."
                                        audio = await synthesize_speech(strip_markdown_for_tts(msg))
                                        if audio and _ws:
                                            try:
                                ...
                                elif embedded_action["action"].startswith("session_"):
                                    async def _handle_session_action(_action=embedded_action["action"], _target=embedded_action["target"], _ws=ws):
                                        result = await _execute_session_action(_action, _target, _ws)
                                        if result:
                                            log.info(f"세션 액션 응답: {result[:80]}")
                                            ...
                                            if audio and _ws:
                                                try:
                                                    ...
                                                    pass
                                    asyncio.create_task(_handle_session_action())
                                elif embedded_action["action"] in ("motion_enable", "motion_disable", "motion_pause", "motion_resume", "motion_calibrate"):
                                    event_map = {
                                        "motion_enable": "motion_control.enable",
                                        "motion_disable": "motion_control.disable",
                                        "motion_pause": "motion_control.pause",
                                        "motion_resume": "motion_control.resume",
                                        "motion_calibrate": "motion_control.calibrate",
                                    }
                                    event_type = event_map[embedded_action["action"]]
                                    asyncio.create_task(motion_controller.handle_event(event_type, {}))
                                    log.info(f"Motion event dispatched: {event_type}")
                                elif embedded_action["action"] == "desktop_goto":
                                    target = embedded_action["target"].strip()
                                    try:
                                        idx = int(target)
                                        asyncio.create_task(motion_controller.handle_event(
                                            "motion.desktop.goto", {"target": idx}
                                        ))
                                    except ValueError:
                                        asyncio.create_task(motion_controller.handle_event(
                                            "motion.desktop.goto", {"target": target}
                                        ))
                                    log.info(f"Desktop goto: {target}")
                                elif embedded_action["action"] in ("hud_show", "hud_hide"):
                                    msg = {"type": "hud_visibility", "visible": embedded_action["action"] == "hud_show"}
                                    asyncio.create_task(motion_controller._broadcast(msg))
                                elif embedded_action["action"] == "launch_hud":
                                    async def _do_launch_hud(_ws=ws):
                                        result = await _launch_hud()
                                        audio = await synthesize_speech(result)
                                        if audio and _ws:
                                            try:
                                                await _ws.send_json({"type": "audio", "data": base64.b64encode(audio).decode(), "text": result})
...
async def api_fix_self():
    """Enter work mode in the DOBBY repo — DOBBY can now fix himself."""
    dobby_dir = str(Path(__file__).parent)
    ...
    script = (
        'tell application "Terminal"\n'
        '    activate\n'
        f'    do script "cd {dobby_dir} && claude --dangerously-skip-permissions"\n'
        'end tell'
    )
    await asyncio.create_subprocess_exec(
        "osascript", "-e", script,
    ...
    log.info("Work mode: DOBBY repo opened for self-improvement")
    return {"status": "work_mode_active", "path": dobby_dir}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DOBBY Server")
